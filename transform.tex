...
\mathbf{F} & \mathbf{0}\\
\mathbf{F}'_{21} & \mathbf{F}'_{22}\end{array}\right]=\left[\begin{array}{l|cr}
...
and $(\mathbf{F}',\vec{\omega},\vec{s'})$-bases in \prettyref{cor:FtauBasisForm}
...
be the top $n$ rows of $\bar{\mathbf{S}}_{\tau}$ for any degree
bound $\tau\in\mathbb{Z}$. Then any $\mathbf{q}\in\left\langle \left(\mathbf{F},\sigma,\vec{s}\right)\right\rangle _{\tau}$
...
to a simple form shown in \prettyref{cor:FtauBasisForm}, which helps
to establish a close correspondence between a $(\mathbf{F}',\vec{\omega},\vec{s'})$-basis
and a $\left(\mathbf{F},\sigma,\vec{s}\right)$-basis in \prettyref{lem:2delta-1Basis},
...
$\deg_{\vec{s'}}\bar{\mathbf{S}}^{\left(2\right)}=2\delta$ and $\deg_{\vec{s'}}\bar{\mathbf{S}}^{\left(1\right)}\le2\delta-1$.
Let $\mathbf{S}^{\left(12\right)},\mathbf{S}^{\left(1\right)},\mathbf{S}^{\left(2\right)}$
...
1+x+x^{2} & x+x^{2}+x^{3} & 1+x+x^{2}+x^{3} & x^{3} & 0\  & 1\end{array}}\right],\end{align*}
 a $\left(\mathbf{F}',\left[8,4,4\right],\vec{0}\right)$-basis is
\[
